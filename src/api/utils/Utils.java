...
public class Utils {
	...
	public static boolean directTeleport(String teleport) {
		...
		if (widget != null) {
			...
			if (actions.length > 0 && actions[actions.length - 1].contains(teleport)) {

				/*
				 * if (widget.click(3)) { ctx.sleep(450); ctx.sleepCondition(()
				 * -> ctx.players.getLocal().getAnimation() != -1, 450);
				 * ctx.sleepCondition(() ->
				 * ctx.players.getLocal().getAnimation() == -1, 550);
				 * ctx.sleep(1500); return true; }
				 */
			}
		}
		return false;
