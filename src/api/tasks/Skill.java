...
public class Skill {
	...
	public void flickPrayer(boolean enable) {
		try {
			synchronized (Variables.scheduledPrayers) {
				Variables.scheduledPrayers.forEach(enable ? this::enablePrayer : this::disablePrayer);
			}
	...
	public void enablePrayer(Prayers prayer) {
		...
		if (!ctx.prayers.prayerActive(prayer)) MenuActions.invoke("Activate",
				...
				57, -1, prayer.getWidgetInfo().getId());
	...
	public void addPrayer(Prayers prayer) {
		if (Variables.scheduledPrayers.contains(prayer)) return;
		Variables.scheduledPrayers.add(prayer);
	}

	public void removePrayer(Prayers prayer) {
		if (Variables.scheduledPrayers.contains(prayer)) {
			Variables.scheduledPrayers.remove(prayer);
		}
