...
public class Combat {
	...
	public SimpleNpc getNPC(boolean filter, int... npcId) {
		return ctx.npcs.populate().filter(npcId).filter(GET_AGGRESSOR).nearest().next();
	}
	...
	public void attack(SimpleNpc npc) {
		...
		try {
			/*
			 * NPC rnpc = (NPC) npc; ctx.log("<col=ffff00>" + npc.getName() +
			 * "<col=ff00>  (" + rnpc.getCombatLevel() + ")" + " - index: " +
			 * rnpc.getIndex()); Variables.invokeMenuAction("Attack",
			 * "<col=ffff00>" + npc.getName() + "<col=ff00>  (" +
			 * rnpc.getCombatLevel() + ")", rnpc.getIndex(), 10, 0, 0);
			 * ctx.sleep(150, 350);
			 */
			MenuActions.click(npc, "Attack");
			ctx.onCondition(() -> ctx.players.getLocal().getInteracting() != null, 1000);
	...
	public void switchSpec() {
		...
		if (weapon != null) {
			...
			MenuActions.click(weapon, "wear");
			ctx.onCondition(() -> Weapons.SPECIAL_WEAPON.itemEquiped());
		}
	}
